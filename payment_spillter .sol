...
import "../token/ERC20/utils/SafeERC20.sol";        // safeerc20 is an upgraded version of erc20 which eliminates the return bug from transfer function 
import "../utils/Address.sol";                      // checks whther acount is contract or not 
import "../utils/Context.sol";                      // provides the user data....It is used when there is 3rd person who verifies the transaction and looks on the gas fee 
...
contract PaymentSplitter is Context {
    ...
    mapping(address => uint256) private _shares;       // address and their shares of the fund
    mapping(address => uint256) private _released;     // claimed tokens
    address[] private _payees;                         // array of payees

    mapping(IERC20 => uint256) private _erc20TotalReleased;  // different tokens and their totalsupply
    ...
    /**
     * for the amount of payee's releasable `token` tokens. `token` should be the address of an
     * IERC20 contract.
     */
    ...
    /**
     * the pending payment of an `account` given the token historical balances and
